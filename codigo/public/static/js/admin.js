...
function delete_object(e){
    ...
    if(action == 'delete'){
        mdalert({title: '¿Esta seguro de eliminar este objeto?', type: 'delete', msg: '¡Recuerde que esta acción enviara este elemento a la papelera o lo eliminara de forma definitiva.!', actions: JSON.stringify([{ url: url, name: 'Sí, eliminar', type: 'danger'  }]) });
    }else{
        mdalert({title: '¿Desea restaurar este objeto?', type: 'restore', msg: '¡Esta acción restaurará el objeto de la papelera de reciclaje.!',actions: JSON.stringify([{ url: url, name: 'Sí, Restaurar', type: 'success'  }]) });
    }
