                    ...
                    <tr>
                        @foreach($products as $p)
                            <tr>
                                <td width="140">
                                    <div class="opts">
                                        @if(kvfj(Auth::user()->permissions, 'product_edit'))
                                            <a href="{{ url('/admin/product/'.$p->id.'/edit') }}" data-toogle="tooltrip" data-placement="top" title="Editar"><i class="fas fa-edit"></i></a>
                                        @endif                                        
                                        @if(kvfj(Auth::user()->permissions, 'product_delete'))
                                            <a href="{{ url('/admin/product/'.$p->id.'/delete') }}" data-toogle="tooltrip" data-placement="top" title="Eliminar"><i class="fas fa-trash-alt"></i></a>
                                        @endif
                                    </div>
                                </td>
                                <td width="50">{{ $p->id }}</td>
                                ...
                                <td>{{ $p->name }} @if($p->status == "0") <i class="fas fa-eraser" data-toogle="tooltrip" data-placement="top" title="Estado: Borrado"></i> @endif</td>
